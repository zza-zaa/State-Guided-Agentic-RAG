...
def infer_question_type_v2(question: str) -> QuestionTypeSpec:
    q = _norm(question)
    ...
    comparison_cues = [
        ...
        " same country", " same nationality", " same conference",
        " which film has the director", " who died first",
        " which writer was from ", " which performance act has "
    ]
    boolean_cues = [
        "are both", "do both", "both ", "same country", "same nationality", "same conference"
    ]

    if any(cue in q for cue in boolean_cues):
    ...
    # avoid false positive like "father of modern American shipbuilding"
    ...
    if any(t in q for t in kinship_terms) and not any(bad in q for bad in false_kinship_phrases):
        ...
        "share a border", "border with", "located in", "located?"
    ]):
    ...
    nested_cues = [
        ...
        "company that distributed", "person who is part of", "father of", "mother of", "child of"
    ]

    if operator_type in {"comparison", "boolean_conjunction"}:
        chain_depth_hint = 2
    elif relation_family == "kinship":
        chain_depth_hint = 3 if any(x in q for x in ["father-in-law", "mother-in-law", "paternal", "maternal"]) else 2
