    ...
    for fp in files:
        try:
            ...
            for rec in iter_records(fp):
                ...
                # musique 常见 paragraph / paragraphs / context 结构
                if "paragraphs" in rec and isinstance(rec["paragraphs"], list):
                    for p in rec["paragraphs"]:
                        if isinstance(p, dict):
                            add_doc(p.get("title", rec.get("title", "")), p.get("text", "") or p.get("paragraph_text", ""), docs)
                        else:
                            add_doc(rec.get("title", ""), p, docs)

                if "context" in rec and isinstance(rec["context"], list):
